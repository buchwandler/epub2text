...
def _fragment_offset(
    doc: SourceDocument | None, fragment: str | None
) -> tuple[int | None, int | None, list[Diagnostic]]:
    ...
    if not match:
        diagnostics.append(
            Diagnostic(
                "warning",
                "unresolved_fragment",
                f"Fragment {fragment!r} was not found",
                doc.href,
            )
        )
        return None, None, diagnostics
...
def navigation_from_parser(
    parser: Any, documents: list[SourceDocument] | None = None
    ...
    try:
        ...
        return [
            NavigationEntry(
                ...
                (),
                "fallback",
                [Diagnostic("warning", "missing_nav", "No navigation document found")],
            )
        ]

    docs_by_href = {doc.href: doc for doc in documents or []}
    docs_by_href.update(
        {urllib.parse.unquote(doc.href): doc for doc in documents or []}
    )
        ...
        for node in nodes:
            ...
            if doc_href and doc is None and documents is not None:
                diagnostics.append(
                    Diagnostic(
                        "warning",
                        "unresolved_href",
                        f"Navigation href {doc_href!r} was not found",
                    )
                )
            ...
            entries.append(
                NavigationEntry(
                    ...
                    title,
                    href,
                    doc.href if doc else doc_href,
                    fragment,
                    doc.spine_index if doc else None,
                    source_start,
                    byte_start,
                    level,
                    parent_id,
    ...
    if child_map:
        entries = [
            entry
            if entry.id not in child_map
            else NavigationEntry(
                **{**entry.__dict__, "children": tuple(child_map[entry.id])}
            )

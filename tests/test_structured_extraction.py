...
def make_epub(path):
    ...
    chapter.content = """<html><body><h1 id="c1">Chapter</h1><p>Hello <em>world</em> &amp; A&nbsp;B<sup>1</sup></p><ol start="4"><li>Alpha</li><li>Beta</li></ol></body></html>"""
...
def test_structured_extraction_blocks_runs_entities_and_json(tmp_path):
    ...
    assert "<p" in extraction.documents[0].text[paragraph.outer_char_start:paragraph.outer_char_end]
    assert "<em>" in [getattr(run, "raw", None) for run in paragraph.runs]
    assert any(getattr(run, "raw", None) == "&amp;" and run.text == "&" for run in paragraph.runs)
    ...
    for segment in extraction.segments:
        ...
        assert block.text[segment.block_text_start:segment.block_text_end] == segment.text

    payload = extraction.to_json(include_raw=False, indent=2)
    assert payload == parser.extract_structured(include_segments=True).to_json(include_raw=False, indent=2)
